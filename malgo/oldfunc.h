#ifndef GXX_MATH_oldmalgo_H
#define GXX_MATH_oldmalgo_H
...
template<typename T, typename O = oldmalgo::math::row_major, typename A = std::allocator<T>> class matrix;
template<typename T, typename O = oldmalgo::math::row_major> class matrix_accessor;

template<typename V>
class vector_basic : public nos::array_printable<V> {
public:	
	template<typename OV> V& operator+=(const OV& oth) { 
		V& self = *static_cast<V*>(this);
		oldmalgo::vector_add(self.begin(), oth.begin(), self.size(), self.begin()); 
		return self;
	}

	template<typename OV> V& operator-=(const OV& oth) { 
		V& self = *static_cast<V*>(this);
		oldmalgo::vector_sub(self.begin(), oth.begin(), self.size(), self.begin()); 
		return self;
	}

	void self_scale(double arg) { 
		V& self = *static_cast<V*>(this);
		oldmalgo::vector_scale(self.begin(), self.size(), arg, self.begin()); 
	}

	void self_normalize() { 
		V& self = *static_cast<V*>(this);
		oldmalgo::vector_normalize(self.begin(), self.size(), self.begin()); 
	}
	...
	auto operator+(const OV& b) const& { 
		...
		oldmalgo::vector_add(self.begin(), b.begin(), self.size(), ret.begin()); 
		return ret; 
	}
	...
	auto operator-(const OV& b) const& { 
		const V& self = *static_cast<const V*>(this);
		vector<decltype(self[0] - b[0])> ret(self.size()); 
		oldmalgo::vector_sub(self.begin(), b.begin(), self.size(), ret.begin()); 
		return ret; 
	}
	...
	void copy(const OV& b) {
		V& self = *static_cast<V*>(this);
		oldmalgo::vector_copy(b.begin(), self.size(), self.begin());
	}
...
public:
	...
	vector(const igris::array_view<T>& buf) : parent(alloc.allocate(buf.size()), buf.size()) { std::copy(buf.begin(), buf.end(), parent::begin()); }
	...
	using parent::operator+;
	vector operator+(vector&& b) const& { vector ret(std::move(b)); oldmalgo::vector_add(parent::begin(), ret.begin(), parent::size(), ret.begin()); return ret; }
	vector operator+(const vector& b) && { vector ret(std::move(*this)); oldmalgo::vector_add(ret.begin(), b.begin(), b.size(), ret.begin()); return ret; }

	using parent::operator-;
	vector operator-(vector&& b) const& { vector ret(std::move(b)); oldmalgo::vector_sub(parent::begin(), ret.begin(), parent::size(), ret.begin()); return ret; }
	vector operator-(const vector& b) && { vector ret(std::move(*this)); oldmalgo::vector_sub(ret.begin(), b.begin(), b.size(), ret.begin()); return ret; }
};
...
template <typename M>
class matrix_basic : public nos::matrix_printable<M> {
public:
	/////////////////////
};
...
public:
	...
	T& operator()(size_t pos1, size_t pos2) { return oldmalgo::math::major_accessor<T*,O>::ref(dat, pos1, pos2, sz1, sz2); }
	const T& operator()(size_t pos1, size_t pos2) const { return oldmalgo::math::major_accessor<const T*,O>::const_ref(dat, pos1, pos2, sz1, sz2); }
	...
	auto operator+(const matrix_compact_basic& b) const& { matrix<T> ret(size1(), size2()); oldmalgo::vector_add_it(begin(), end(), b.begin(), ret.begin()); return ret; }
	auto operator+(matrix_compact_basic&& b) const& { matrix<T> ret(std::move(b)); oldmalgo::vector_add_it(begin(), end(), ret.begin(), ret.begin()); return ret; }
	auto operator+(const matrix_compact_basic& b) && { matrix<T> ret(std::move(*this)); oldmalgo::vector_add_it(ret.begin(), ret.end(), b.begin(), ret.begin()); return ret; }

	auto operator-(const matrix_compact_basic& b) const& { matrix<T> ret(size1(), size2()); oldmalgo::vector_sub_it(begin(), end(), b.begin(), ret.begin()); return ret; }
	auto operator-(matrix_compact_basic&& b) const& { matrix<T> ret(std::move(b)); oldmalgo::vector_sub_it(begin(), end(), ret.begin(), ret.begin()); return ret; }
	auto operator-(const matrix_compact_basic& b) && { matrix<T> ret(std::move(*this)); oldmalgo::vector_sub_it(ret.begin(), ret.end(), b.begin(), ret.begin()); return ret; }

	auto dot(const matrix_compact_basic& b) const& { 
		if (size2() != b.size1()) return matrix<T>();
		matrix<T> ret(size1(), b.size2()); 
		oldmalgo::matrix_compact_mux(begin(), b.begin(), size1(), size2(), b.size2(), ret.begin()); 
		return ret; 
	}
	...
	void serialize(M& r) const {
		r & sz1;
		r & sz2;
		r & igris::archive::data<T>{ dat, sz1 * sz2 };	
	}
...
public:
	...
	matrix(const OM& oth) : parent(alloc.allocate(oth.sz1*oth.sz2), oth.sz1, oth.sz2) {
		oldmalgo::vector_copy_it(oth.begin(), oth.end(), parent::begin());
	}

	//matrix(const matrix& oth) : parent(alloc.allocate(oth.sz1*oth.sz2), oth.sz1, oth.sz2) {
	//	oldmalgo::vector_copy(oth.begin_row(), oth.end_row(), parent::begin_row());
	//}
	...
	void deserialize(M& r) {
		...
		r & igris::archive::data<T>(parent::dat, parent::sz1 * parent::sz2);	
	}
};
...
void vector_random( T* A, int n, T min, T max) {
	while(n--) { 
		*A++ = igris::random_minmax<T>(min, max);
		//gxx::println(*(A-1));
	}
}
...
inline float vector_quick_invabs(float* v, size_t n) {
	return quick_invsqrt(vector_abs(v,n));
}
...
void matrix_copy_by_rows(I1 A, I1 EA, I2 B) {
	while(A != EA) {
		oldmalgo::vector_copy_it(A.begin(), A.end(), B.begin());
		++A; ++B;
	}
}
